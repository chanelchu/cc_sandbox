...
class AdverseImpact_Race(object):
    ...
    def JFF_Color_N(self, raw_data):
        ...
        df['jff_color'] = 'N_'+df['color']+'_'+df['jff']
        ...
        for x in race:
            for i in colnames:
                raw_data['N_GREEN_%s' % i] = df.N[df.jff_color == 'N_GREEN_%s' % i]
                raw_data['N_YELLOW_%s' % i] = df.N[df.jff_color == 'N_YELLOW_%s' % i]
                raw_data['N_RED_%s' % i] = df.N[df.jff_color == 'N_RED_%s' % i]
        

        colnames1 = list(raw_data.columns.values)
        del colnames1[:5]
        
        for x in race:
            for y in colnames1:
                raw_data['%s' % y][raw_data.race == x] = df.N[(df.jff_color == '%s' % y) & (df.race == '%s' % x)]
        '''        
        return raw_data
    ...
    def order(self):
        data = self.raw_data()
        data2 = self.pass_rate(data)
        data3 = self.threshold(data2)
        print data3
        return data3
    '''   
